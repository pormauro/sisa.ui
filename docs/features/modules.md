...
- Los campos `company_id` y `user_id` siempre deben enviarse explícitamente porque la base `sisa.api` continúa sin claves foráneas: los formularios y el contexto replican este requisito incluso cuando la operación parte de un flujo anidado (`/companies/{id}/memberships`). Además, el parser agrega `message`, `notes`, `role`, `reason`, `responded_by_id`, `responded_by_name`, `responded_at` y `audit_flags` aunque el backend devuelva variantes (`request_message`, `membership_role`, `rejection_reason`, etc.) para que todas las pantallas puedan mostrar y editar el historial completo.【F:contexts/CompanyMembershipsContext.tsx†L25-L188】
...
### Métodos del contexto
- `loadCompanyMemberships(companyId?)`: al recibir un ID usa `GET /companies/{companyId}/memberships` para refrescar solo esa empresa; cuando se invoca sin argumentos recorre todas las compañías donde el usuario figura en `admin_users` (o es superadministrador) y ejecuta la misma consulta una a una para poblar el listado completo.【F:contexts/CompanyMembershipsContext.tsx†L780-L983】
...
- Para tolerar instalaciones que solo exponen la variante legacy `/company-memberships`, el contexto memoriza automáticamente el primer endpoint que respondió distinto de `404` y reutiliza ese path al construir la URL del stream, evitando los fallos de WebSocket/SSE apenas se inicia la aplicación.【F:contexts/CompanyMembershipsContext.tsx†L425-L520】【F:contexts/CompanyMembershipsContext.tsx†L820-L950】
...
- `POST ${BASE_URL}/companies/{companyId}/memberships` — flujo anidado para que un usuario solicite acceso a una empresa concreta; el `CompanyMembershipsContext` detecta automáticamente esta variante y serializa `message`, `role`, `notes` y los IDs requeridos antes de golpear el endpoint.【F:contexts/CompanyMembershipsContext.tsx†L600-L748】
- `POST ${BASE_URL}/companies/{companyId}/memberships/{membershipId}/approve` y `POST ${BASE_URL}/companies/{companyId}/memberships/{membershipId}/reject` — endpoints de aprobación/rechazo que incluyen campos opcionales como `notes`, `message`, `role`, `reason`, `responded_at` y banderas de auditoría. El provider intenta estas rutas y, si recibe un `404`, cae automáticamente en el `PUT /company_memberships/{id}` legacy para mantener compatibilidad con instalaciones antiguas.【F:contexts/CompanyMembershipsContext.tsx†L520-L830】
- `/company_memberships/stream` (o la variante con guion bajo) solo se consume tras verificar con un `HEAD` inicial que el recurso existe. Si el backend responde `404/501` o no soporta el stream, el provider deja únicamente la sincronización periódica y muestra una única advertencia ligera para informar que continúa en modo polling, evitando errores visibles en servidores estrictamente REST.【F:contexts/CompanyMembershipsContext.tsx†L434-L520】【F:contexts/CompanyMembershipsContext.tsx†L820-L960】
- El stream se habilita únicamente después de detectar cuál de los endpoints (`/company_memberships` vs `/company-memberships`) respondió exitosamente durante las primeras cargas; hasta entonces, el provider mantiene el modo polling y evita iniciar conexiones SSE/WebSocket con rutas desconocidas.【F:contexts/CompanyMembershipsContext.tsx†L598-L978】
...
- El flujo `requestMembershipAccess` permite que cualquier usuario autenticado (aunque no posea `addCompanyMembership`) envíe una solicitud desde `app/company_memberships/request.tsx`; si el backend no soporta los endpoints anidados, la interfaz reutiliza el `POST /company_memberships` tradicional con los mismos campos para garantizar el fallback. Las resoluciones (`approve`/`reject`) respetan los permisos administrativos, pero ante la falta de los endpoints anidados se delega en `updateCompanyMembership` con `PUT /company_memberships/{id}` manteniendo la traza de `message`, `notes`, `reason` y `responded_at`.【F:app/company_memberships/request.tsx†L24-L212】【F:contexts/CompanyMembershipsContext.tsx†L293-L918】
