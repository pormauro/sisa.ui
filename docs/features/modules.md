...
- `GET ${BASE_URL}/companies/member` — devuelve las empresas donde el usuario tiene membresías vigentes y permite filtrar por `status` y `role` (ej. `?status=approved&role=owner,admin,member`), respetando el permiso `listMemberCompanies`.【F:contexts/CompaniesContext.tsx†L1045-L1110】
