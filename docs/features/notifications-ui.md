...
- Los botones “Marcar como leída” y “Ocultar” solo aparecen si el usuario tiene los permisos `markNotificationRead` o `hideNotification` (o es el superusuario). Ambas acciones actualizan el caché local y disparan los endpoints `PATCH /notifications/{id}/read` y `PATCH /notifications/{id}/hide` respectivamente.【F:app/notifications/index.tsx†L86-L112】【F:app/notifications/index.tsx†L151-L179】
- Si existen pendientes y el usuario posee `markAllNotificationsRead`, el CTA “Marcar N como leídas” invoca `POST /notifications/mark-all-read` y luego recarga el listado actual.【F:app/notifications/index.tsx†L159-L214】
