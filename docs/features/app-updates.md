...
`useCachedState` y vuelve a consultar cuando hay token disponible. Para evitar que la verificación bloquee la carga, se aplica una ventana de 6 horas entre chequeos a menos que se llame a `refreshLatestUpdate({ force: true })`. El estado expone `updateAvailable`, `latestUpdate`, la
`currentVersion` tomada de la configuración de Expo y la marca `lastCheckedAt` para depuración.【F:contexts/AppUpdatesContext.tsx†L6-L107】
