...
- **Release 1.3.6 (monitor de solicitudes de red):** la `APP_VERSION` ahora es `1.3.6` y se agrega `/network/logs` al menú de Configuración para auditar peticiones con filtros por método, estado y URI, paginación incremental y borrado completo mediante confirmación; requiere `listNetworkLogs`, excepto para el superusuario (`userId === '1'`).【F:config/Index.ts†L1-L3】【F:app/network/logs.tsx†L1-L268】【F:docs/menu/sections-proposal.md†L33-L37】
