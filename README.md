...
## Descripción general
SISA es la aplicación móvil y web de gestión operativa de Depros. Unifica la agenda de trabajos, la relación con clientes y proveedores, la gestión documental y los circuitos financieros (recibos, pagos, cajas, tarifas) sobre una única experiencia construida con [Expo Router](https://expo.dev/router) y React Native. El frontend consume la API `sisa.api` y prioriza el trabajo híbrido online/offline combinando `AsyncStorage` (datos en caché) y `expo-file-system` (archivos descargados con metadatos cacheados en el cliente), con sincronización diferencial de operaciones.
...
- `AsyncStorage` + hooks propios (`useCachedState`) para persistencia de datos y `expo-file-system` con metadatos en caché para archivos descargados con rutas determinísticas.
...
- `FilesContext`: subida (`/files`), descarga y cacheo de metadatos/archivos con `expo-file-system` y `AsyncStorage`, incluyendo utilidades para limpiar almacenamiento local.
...
- Los adjuntos se almacenan en `expo-file-system` dentro de `documentDirectory/files/{id}/` con renombrado incremental para evitar colisiones; `FilesContext` reutiliza copias locales cuando es posible y mantiene metadatos sincronizados en `AsyncStorage` para resolverlos offline.
...
- **Release 1.3.9 (bootstrap offline de datos críticos):** la `APP_VERSION` ahora es `1.3.9` y el arranque de la app queda orquestado por `BootstrapProvider`, que hidrata cachés de empresas, membresías, permisos, categorías e ingresos/egresos antes de mostrar `/Home`; si las peticiones fallan, la UI continúa con los datos persistidos. La guía de caché documenta el flujo de arranque y la pantalla principal espera a que el bootstrap termine.【F:config/Index.ts†L1-L3】【F:contexts/BootstrapContext.tsx†L1-L83】【F:app/_layout.tsx†L39-L172】【F:docs/architecture/state-and-cache.md†L95-L116】
- **Release 1.3.8 (registro de red con verbos completos):** la `APP_VERSION` ahora es `1.3.8`, el sniffer normaliza el método en mayúsculas y discrimina por método + URL al evitar duplicados, con lo que los eventos `POST`/`PUT`/`DELETE`/`PATCH` vuelven a aparecer en el historial. La guía se actualiza para reflejarlo.【F:config/Index.ts†L1-L3】【F:utils/networkSniffer.ts†L1-L145】【F:docs/features/network-logs.md†L1-L20】
