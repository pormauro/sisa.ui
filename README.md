...
## Novedades
- **Release 1.4.0 (panel de token exclusivo para superusuario):** la `APP_VERSION` se actualiza a `1.4.0` e incorpora la vista `/settings/auth-diagnostics`, visible solo para el usuario ID 1, con barras de progreso para los timers de autenticación, variables del contexto en vivo y los valores cacheados en SecureStore/AsyncStorage, además de exponer los parámetros activos de reintentos y expiración.【F:config/Index.ts†L1-L3】【F:app/settings/auth-diagnostics.tsx†L1-L243】【F:contexts/AuthContext.tsx†L1-L211】【F:constants/menuSections.ts†L69-L83】
- **Release 1.3.6 (menú inspirado en la referencia con barras superior e inferior):** incorporó un encabezado amarillo con saludo, acceso directo a configuración y tarjeta de empresa activa, además de una barra inferior plana con navegación a Inicio, Avisos, Empresa, Perfil y Ajustes para replicar la experiencia solicitada.【F:app/Home.tsx†L13-L188】【F:components/BottomBubbleBar.tsx†L1-L207】
- **Release 1.3.5 (cajas con asignaciones y permisos administrativos):** se alinea con el release declarado en `sisa.api` (`config/version.json`), manteniendo los campos `assigned_user_ids` y `admin_permissions` para documentar quién puede operar o administrar cada caja; revisa la colección de Postman y la pantalla de permisos para habilitarlos con autenticación Bearer.【F:docs/features/modules.md†L330-L367】【F:app/permission/PermissionScreen.tsx†L53-L82】【F:docs/postman/Sistema.postman_collection.json†L2213-L2266】
