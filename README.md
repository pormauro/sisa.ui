...
## Novedades
- **Release 1.3.6 (menú inspirado en la referencia con barras superior e inferior):** la `APP_VERSION` ahora es `1.3.6` e incorpora un encabezado amarillo con saludo, acceso directo a configuración y tarjeta de empresa activa, además de una barra inferior plana con navegación a Inicio, Avisos, Empresa, Perfil y Ajustes para replicar la experiencia solicitada.【F:config/Index.ts†L1-L3】【F:app/Home.tsx†L13-L188】【F:components/BottomBubbleBar.tsx†L1-L207】
- **Release 1.3.5 (cajas con asignaciones y permisos administrativos):** la `APP_VERSION` ahora es `1.3.5` y se alinea con el release declarado en `sisa.api` (`config/version.json`), manteniendo los campos `assigned_user_ids` y `admin_permissions` para documentar quién puede operar o administrar cada caja; revisa la colección de Postman y la pantalla de permisos para habilitarlos con autenticación Bearer.【F:config/Index.ts†L1-L3】【F:docs/features/modules.md†L330-L367】【F:app/permission/PermissionScreen.tsx†L53-L82】【F:docs/postman/Sistema.postman_collection.json†L2213-L2266】
