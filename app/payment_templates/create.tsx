...
export default function CreatePaymentTemplateScreen() {
  const router = useRouter();
  const { permissions } = useContext(PermissionsContext);
  const { addPaymentTemplate } = useContext(PaymentTemplatesContext);
  ...
  useEffect(() => {
    if (!permissions.includes('addPaymentTemplate')) {
      Alert.alert('Acceso denegado', 'No tienes permiso para crear plantillas de pago.');
      router.back();
    }
  }, [permissions, router]);
  ...
  const handleSubmit = async () => {
    ...
    setLoading(false);
    if (!result) {
      Alert.alert('Error', 'No se pudo crear la plantilla. Inténtalo nuevamente.');
      return;
    }

    Alert.alert('Plantilla creada', 'La plantilla se guardó correctamente.', [
      {
        text: 'Aceptar',
