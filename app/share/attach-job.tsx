...
export default function AttachJobScreen() {
  ...
  const handleAttach = async (jobId: number) => {
    if (!sharedUri) {
    ...
    try {
      ...
      if (!targetJob) {
        await loadJobs();
        targetJob = jobs.find(job => job.id === jobId);
      }
      ...
      const updatedAttachedFiles = [...parseAttachedFiles(targetJob.attached_files), uploaded.id];

      const updated = await updateJob(jobId, {
        client_id: targetJob.client_id,
        description: targetJob.description,
        start_time: targetJob.start_time,
        end_time: targetJob.end_time,
        type_of_work: targetJob.type_of_work,
        status_id: targetJob.status_id,
        folder_id: targetJob.folder_id,
        product_service_id: targetJob.product_service_id,
        multiplicative_value: targetJob.multiplicative_value,
        tariff_id: targetJob.tariff_id,
        manual_amount: targetJob.manual_amount,
        attached_files: updatedAttachedFiles,
        participants: targetJob.participants,
        job_date: targetJob.job_date,
      ...
      <FlatList
        ...
        renderItem={({ item }) => (
          <Pressable
            ...
            onPress={() => void handleAttach(item.id)}
            disabled={loading}
          >
