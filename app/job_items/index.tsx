...
export default function JobItemsScreen() {
  ...
  const persistReorder = async (orderedItems: JobItem[]) => {
  ...
  const renderItem = ({ item, index }: { item: JobItem; index: number }) => (
    <View style={[styles.row, { borderColor }]}>
      ...
      <View style={styles.actions}>
        ...
        {permissions.includes("updateJobItem") && (
          <TouchableOpacity
            onPress={() =>
              router.push(`/job_items/${item.id}?job_id=${item.job_id}`)
            }
          >
  ...
  if (!canListJobItems) {
    return (
      <ThemedView style={styles.container}>
        ...
        <ThemedText style={{ color: textColor }}>
          No tienes permiso para ver los items del trabajo.
        </ThemedText>
      </ThemedView>
    );
  }
    ...
    <ThemedView style={styles.container}>
      ...
      {canReorderJobItems && (
        <ThemedText style={[styles.helpText, { color: textColor }]}>
          Usa las flechas para reordenar los items.
        </ThemedText>
      )}
...
const styles = StyleSheet.create({
  ...
  reorderButton: { color: "#6b7280", fontWeight: "700", fontSize: 18 },
