...
class FolderTreeResource {
  ...
  async load(): Promise<FolderTree> {
    try {
      ...
      if (!clientsResponse.ok) {
        if (isAuthErrorStatus(clientsResponse.status)) {
          console.warn(
            "Auth error while fetching clients for folder tree; using cached data.",
          );
          return this.tree;
        }
      ...
      if (!foldersResponse.ok) {
        if (isAuthErrorStatus(foldersResponse.status)) {
          console.warn(
            "Auth error while fetching folders for folder tree; using cached data.",
          );
          return this.tree;
        }
      ...
      const folders = Array.isArray(foldersData.folders)
        ? foldersData.folders
        : [];
      const folderNodes: FolderTreeNode[] = folders.map((folder) => ({
        id: folder.id.toString(),
        type: "folder",
        parentId: folder.parent_id
          ? folder.parent_id.toString()
          : folder.client_id
            ? folder.client_id.toString()
            : null,
        fileId: folder.folder_image_file_id ?? null,
        name: folder.name,
        children: [],
      }));
      ...
      const tree: FolderTree = [...clientNodes];
      folderNodes.forEach((node) => {
   ...
   */
  async addFolder(
    newFolderData: Record<string, unknown>,
    parentId: string | null,
    ...
    try {
      ...
      if (!response.ok) {
        if (isAuthErrorStatus(response.status)) {
          console.warn(
            "Auth error while adding folder; request will be retried automatically.",
          );
          return null;
        }
      ...
      const addNodeToTree = (nodes: FolderTreeNode[]): boolean => {
  ...
  async updateFolder(
    ...
    try {
      ...
      if (!response.ok) {
        if (isAuthErrorStatus(response.status)) {
          console.warn(
            "Auth error while updating folder; request will be retried automatically.",
          );
          return null;
        }
        const errorData = await response.json();
      ...
      const updateNodeRecursively = (nodes: FolderTreeNode[]): boolean => {
        for (const node of nodes) {
      ...
      return {
        ...
        parentId: updatedFolder.parent_id
          ? updatedFolder.parent_id.toString()
          : updatedFolder.client_id
            ? updatedFolder.client_id.toString()
            : null,
        fileId: updatedFolder.folder_image_file_id ?? null,
  ...
  async deleteFolder(folderId: string | number): Promise<boolean> {
    try {
      ...
      if (!response.ok) {
        if (isAuthErrorStatus(response.status)) {
          console.warn(
            "Auth error while deleting folder; request will be retried automatically.",
          );
          return false;
        }
        const errorData = await response.json();
      ...
      const removeNodeRecursively = (nodes: FolderTreeNode[]): boolean => {
